...
function routes(app) {
	...
	if (this.get('auth') === true) {
		...
		app.all(this.get('resetpassword url'), require('../../admin/routes/views/resetpassword'));
		
		if(!this.get('changepassword url')){
			this.set('changepassword url', '/keystone/changepassword');
		}
		app.all(this.get('changepassword url')+'/:token', require('../../admin/routes/views/changepassword'));
